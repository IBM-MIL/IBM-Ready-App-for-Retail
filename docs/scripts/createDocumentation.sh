...
SWIFT_RELATIVE_PATH_PROJECT="./../../iOS/ReadyAppRetail/"
...
MOBILE_FIRST_JS_RELATIVE_BASE_PATH="./../../ReadyAppsSummit"
...
declare -a mobileFirstJSArray=(
    $MOBILE_FIRST_JS_RELATIVE_BASE_PATH/adapters/SummitAdapter
...
if [ $PRINT_HELP == YES ]; then
    ...
    echo -e "Available command line arguments:"
...
if [ $GEN_JAVA == YES ]; then
    rm -rf ./../gen/Java
    mkdir ./../gen/Java
    rm -rf ./../gen/MobileFirst/Java
    mkdir ./../gen/MobileFirst/Java
...
if [ $GEN_JAVASCRIPT == YES ]; then
    ...
    rm -rf ./../gen/MobileFirst/Javascript
    mkdir ./../gen/JavaScript
    mkdir ./../gen/MobileFirst/Javascript
...
#Added filter to remove warnings about unlinked documentation to the 2 oxygen commands below
if [ $GEN_JAVA == YES ]; then
    doxygen doxygen_config | grep -v 'unable to resolve link'
    echo -e "${GREEN_COLOR}Successfully generated Java documentation with Doxygen ${NO_COLOR}"

    doxygen doxygen_config_mobileFirst | grep -v 'unable to resolve link'
...
if [ $GEN_JAVASCRIPT == YES ]; then
    for path in "${mobileFirstJSArray[@]}"
        do
        if [ -d "$path" ]; then
            jsdoc $path -r -d ./../gen/MobileFirst/JavaScript
            return_code=$?
            if [[ $return_code == 0 ]] ; then
